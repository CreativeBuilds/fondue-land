...
interface IFondueKeys is IERC1155 {
    function ENTRIES_OPEN() external returns (bool);
}

contract TheFondueMicePot is ERC20Rewards, ERC1155Receiver {
    
    IFondueKeys FondueKeys;
    ...
    constructor( IFondueKeys _fondueKeys) ERC20Rewards(
        "fondue",
        "fondue",
        9
    ) {
    ...
    function enterKeys(uint256 _amount) external returns (uint256 keys_deposited){
        require(FondueKeys.ENTRIES_OPEN(), "Entries are closed");
    ...
    function setPaymentToken(IFondueKeys _token) auth public {
